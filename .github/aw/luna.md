...
```bash
gh aw run .github/aw/luna.md \
  --input "Luna, genera la imagen de portada para el artículo sobre la misión lunar"
```
...
```bash
gh aw run .github/aw/luna.md \
  --input "Luna, muéstrame el prompt que generarías para el artículo sobre Marte, sin crear la imagen"
```
...
Una vez mergeado el PR de Luna, push a main activa automáticamente a Rocket para el deploy.
